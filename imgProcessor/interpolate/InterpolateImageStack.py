...
# TODO: make pure python interpolatioj of cubic interpolation
# checkout:
# https://github.com/joonro/fast-cubic-spline-python/blob/master/fast_cubic_spline.py
...
class LinearInterpolateImageStack(object):
    ...
    def __call__(self, val):
        if val == 0:
            return self.imgs[0]
        if val >= len(self.imgs) - 1:
            return self.imgs[-1]
#         ind = np.argmax(self.z >= val)
#         print(ind, val, 8888888888888)
#
#         if ind == 0:
#             return self.imgs[0]
#         elif ind == -1 or ind == len(self.z)-1:
#             return self.imgs[-1]
        ind = int(val)
#         z0, z1 = self.z[ind - 1:ind + 1]
#         dz = z1 - z0
        f = val - ind  # ) / dz
        return (1 - f) * self.imgs[ind] + f * self.imgs[ind + 1]
...
if __name__ == '__main__':
    ...
    zz = np.sort(np.random.rand(4) * len(imgs))
    # interpolated images:
    ii = [I(z) for z in zz]
    print(time() - t0)
