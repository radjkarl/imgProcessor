...
class PerspectiveCorrection(object):
    ...
    def distort(self, img, rotX=0, rotY=0, quad=None):
        ...
        h = wquad[:, 1].max() - wquad[:, 1].min()
        #(int(w),int(h))
        dist = cv2.warpPerspective(corr, homography, (int(w), int(
            h)), flags=cv2.INTER_CUBIC | cv2.WARP_INVERSE_MAP)
    ...
    def correctGrid(self, img, grid):
        ...
        for ix in range(n0):
            for iy in range(n1):
                quad = grid[ix:ix + 2, iy:iy +
                            2].reshape(4, 2)[np.array([0, 2, 3, 1])]
...
#         h = np.linalg.inv(h)
    ...
    def camera_position(self):
        ...
        if self._camera_position is None:
            self._camera_position = - \
                np.matrix(cv2.Rodrigues(r)[0]).T * np.matrix(t)
    ...
    def standardUncertainties(self, focal_Length_mm, f_number,
        ...
        worldCoord = np.fromfunction(
            lambda x, y: imgPointToWorldCoord(
                (y, x), r, t, cam), s)
        depthMap = np.linalg.norm(
            worldCoord - self.camera_position,
            axis=0).reshape(s)
        del worldCoord
    ...
    def draw3dCoordAxis(self, img=None, thickness=8):
        ...
        cv2.line(
            img, origin, tuple(
                imgpts[3].ravel()), (mx, 0, 0), thickness * 2)
        return img
