...
def plotFitResult(fit, show_legend=True, show_plots=True, limitXRangeGaussPlots=True, save_to_file=False, foldername='', filename='', filetype='png'):
    from matplotlib import pyplot
    a,b,c = fit.fitParams[-1]
    if limitXRangeGaussPlots:
        #limit x range:

        border = b+3*c
        #find index of xvals outside the border
        for n,x in enumerate(fit.xvals):
            if x > border:
                break 
    else:
        n = None
...
if __name__ == '__main__':
    ...
    for i in imgs:
        ...
        print f.fitParams
        plotFitResult(f, limitXRangeGaussPlots=True, save_to_file=False)